...
proc check10(SyntaxTree:COMMAND[], SymTabIn :string[] ->
   ...
   rule check10(COMMAND[], SymTab -> SymTab, 0):
        
   rule check10(COMMAND[dec(ID)::T], TabIn -> TabOut2, NE+NE2):
      doubleDecs(ID, TabIn -> TabOut, NE)
      check10(T, TabOut -> TabOut2, NE2)
        
   rule check10(COMMAND[stm(H)::T], TabIn -> TabOut, NE+NE2):
      undeclared(H, TabIn -> NE)
      check10(T, TabIn -> TabOut, NE2)        
...
proc doubleDecs(Ident: string, SymTabIn :string[] ->
   ...
   rule doubleDecs(ID, TabIn -> TabIn, 1):
        isDeclared(ID, TabIn -> ID2)
        sourcepos(ID -> EL)
        sourcepos(ID2 -> EL2)
        "Double declaration: " $ID " is declared at line " pLine(EL) " and " pLine(EL2) "\n"
   rule doubleDecs(ID, TabIn -> string[ID::TabIn], 0):


proc undeclared(Ident: string, SymTab:string[] -> NrOfErrors: int)
   // Checks if the Ident is in the SymTab (if not, Ident is undeclared)

   rule undeclared(ID, string[] -> 1):
   rule undeclared(ID, ST -> 0):
       isDeclared(ID, ST -> IdOut)
   rule undeclared(ID, ST -> 1):
       sourcepos(ID -> EL)
       "Undeclared variable: " $ID " at line " pLine(EL) " is not declared\n"
...
condition isDeclared(IdentIn: string, SymTab:string[] -> IdentOut:string)
   ...
   rule isDeclared(ID, string[H::T] -> H):
            Equal(ID, H)
            // sourcepos(ID -> SP1)
            // sourcepos(H -> SP2)
            // Equal(SP1, SP2)
   rule isDeclared(ID, string[H::T] -> X):
            isDeclared(ID, T -> X)
