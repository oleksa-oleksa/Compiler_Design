#!/bin/bash
#
# Script b.cmd (executes one of the ancestor compilers)
# ----------------------------------------------------------------------
...
# > b                          Die Mutter von Maria
# > b Der Vater von Johann     Der Vater von Johann
...
if [ -z "${1}" ]; then
   echo Die Mutter von Maria > word.tmp   # If no parameter was passed
else
   if [ -f "${1}" ]; then
