...
public class Llana
{
   ...
   private static ArrayList<Nonterm> NontermCollection;

   
   public static void CollectNonterms()
   {
      Nonterm rt = (Nonterm) ExhaustiveParser.UserRoot;
      NontermCollection = new ArrayList<Nonterm>();
      Rec(rt);
      for (Nonterm n : NontermCollection) {
         P("collected "+n.Name);
         for (Rule r : n.Rules) {
            r.Print();
         }
   ...
   private static void Rec(Nonterm n)
   {
      if (NontermCollection.contains(n)) return;
      
      NontermCollection.add(n);
      
      for (Rule r : n.Rules) {

         for (Symbol s: r.Rhs) {

            if (s instanceof Nonterm) Rec((Nonterm) s);
         }
      }
   }
